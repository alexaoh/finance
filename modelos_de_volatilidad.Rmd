...
Still, the parameter estimates are significant (except for `omega`). The stationarity conditions holds, since $\hat{\alpha}_1 + \hat{\beta}_1 + \frac12 \hat{\gamma} \approx$ `r round(0.823974 + 0.5*0.260477, 3)` $<1$. However, the positivity condition does not seem to hold, since $\hat{\alpha} = 0 \ngeq 0$. Thus, the GJR-GARCH based model cannot be used, even though the AIC is lower and the residuals do not present any autocorrelation according to plots. 
...
NOT SURE HOW TO DO THIS YET. DOES NOT LOOK LIKE IT IS IMPLEMENTED!

Next, we will fit an ARMA(2,2)-EGARCH model. All parameters are significant in the model. 
...
The residual plots below look good. Moreover, the AIC is smaller for this model compared to the first proposed model. Thus, I would prefer this model over the first model. 
...
The residuals for the iGARCH look alright, but the AIC is lower for this model compared to the first proposed model. 
...
Thus, I would conclude that the best model out of the three fitted is the first one. 

# Grafic and Interpretation of the Estimated Series of Volatility

The estimated series of volatility for the first model is shown below, together with the returns. 
PERHAPS THE E-GARCH IS BETTER THEN!?
...
The general behaviour seems to match relatively well, i.e. the movements in the two plots coincide relatively well.
...
The news impact curve for our chosen model is shown below. Since the standard GARCH model does not take the leverage effect into effect, the news impact curve is symmetric. DETTE ER JO DUMT, MEN KAN JO IKKE BRUKE GJR-GARCH VIRKER DET SOM!? NOE ANNET JEG KAN GJØRE MED DETTE?
...
Here we will calculate and interpretate the Value at Risk (VaR) using estimated volatilities from several different models. 

# Multivariate DCC

Should choose a stock in the NASDAQ, perhaps Apple (?) and compare to the NASDAQ Composite. 
