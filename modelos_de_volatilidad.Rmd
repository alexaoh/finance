...
Here we will calculate and interpret the Value at Risk (VaR) using estimated volatilities from several different models. First we use the variance-covariance method, calculating the VaR with a static forecast one time ahead, using the EGARCH model. Thus, this is an out-of-sample estimation of the VaR. 
...
Next we calculate the VaR with a rolling window dynamic forecast (still out-of-sample), using the EGARCH model, with a significance level of 5\%. 
...
It is apparent that the EGARCH-based model severely underestimates the necessary funds to avoid exceeding the VaR, which is an indication that the model is not very well suited for use in practice, at least not for calculating VaR's. SJEKK AT JEG HAR SKREVET/ARGUMENTERT KORREKT. Because of this indication, we will try to do the same calculations with the GJR-GARCH-based model also, to see if the results are more promising. IN-SAMPLE ESTIMERINGEN VIL JEG AT SKAL KOMME FØR DETTE (OG FØR OUT OF SAMPLE!). 

Next, we calculate the VaR using estimates of volatility from the EWMA model with $\lambda = 0.75$ and from the ARMA(2,2)-EGARCH model. Thus, this is an in-sample comparison of the two models when calculating volatility and VaR. THIS IS IN SAMPLE, PERHAPS DO THIS FIRST!!!
...
* Value of gamma in EGARCH: Opposite of how we defined it? Se kommentarer i selve oppgaven (side 19/20). IN GENERAL: HOW CAN THE PARAMETERS OF THE EGARCH ESTIMATED HERE BE INTERPRETED? THEY USE A DIFFERENT PARAMETRIZATION THAN WHAT WE DID IN SLIDES, SEE DOCS!
...
* Why does the EGARCH VaR exceed the 1\% level when using 0.01 as a significance level?
* Interpretations in the multivariate case: From the correlation and from the news impact surface?